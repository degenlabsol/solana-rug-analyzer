...
const config = {
  telegram: {
    ...
    // Support both SESSION_SECRET (Replit) and TELEGRAM_SESSION (self-hosted)
  ...
  api: {
    ...
    // Support both BIRDEYE_API (Replit) and BIRDEYE_API_KEY (self-hosted)
    birdeyeKey:     process.env.BIRDEYE_API        ||
                    process.env.BIRDEYE_API_KEY    || '',
    // Support both OPENROUTER_KEY (Replit) and OPENROUTER_API_KEY (self-hosted)
    openRouterKey:  process.env.OPENROUTER_KEY     ||
                    process.env.OPENROUTER_API_KEY || '',
    openRouterModel: process.env.OPENROUTER_MODEL  || 'mistralai/mistral-7b-instruct:free',
    // Additional APIs present in this Replit env
...
function validate() {
  ...
  if (!config.telegram.session)
    errors.push('SESSION_SECRET (or TELEGRAM_SESSION) is missing — bot cannot log in');
